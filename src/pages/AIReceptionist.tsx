
import React from 'react';
import Navbar from '@/components/layout/Navbar';
import Footer from '@/components/layout/Footer';
import { HeroSection } from '@/components/voice-chat/page-sections/HeroSection';

const AIReceptionist = () => {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a0b2e] to-[#13151a]">
      <Navbar />
      <main className="container mx-auto px-4 py-8">
        <HeroSection 
          title="AI Receptionist"
          subtitle="Intelligent virtual receptionists for your business"
          description="Our AI receptionists handle calls, schedule appointments, and answer customer inquiries 24/7 with human-like conversation capabilities."
        />
        <div className="my-16 text-center">
          <h2 className="text-3xl font-bold text-white mb-8">Coming Soon</h2>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            We're currently developing our AI Receptionist solution. Check back soon for more information or contact us for early access.
          </p>
        </div>
      </main>
      <Footer />
    </div>
  );
};

export default AIReceptionist;
